:- encoding(utf8).
% Gramática para analizador de frases en castellano
...
% Regla principal.
% Se aplica unificación para la concordancia: pasamos la variable 'Num'
% desde el sujeto al predicado para asegurar que singular va con singular, etc.
% Vamos guardando la estructura en o(Suj, Pred, Punc) para generar el árbol.
...
% Dividimos el SN en 'base' (art+nombre) y una lista recursiva de complementos.
sintagma_nominal(Num, Gen, sn(Det, N)) -->
    sintagma_nominal_base(Num, Gen, Det, N),
    complementos_nominales(Num, Gen, []).

% Caso en el que sí hay complementos. La variable 'Complements' captura la lista.
sintagma_nominal(Num, Gen, sn(Det, N, Complements)) -->
    sintagma_nominal_base(Num, Gen, Det, N),
    complementos_nominales(Num, Gen, Complements),
    { Complements \= [] }.

% Nombres propios y pronombres actúan como un SN completo
...
% Base del SN: Fuerza la concordancia de Num y Gen entre artículo y nombre.
sintagma_nominal_base(Num, Gen, Det, N) -->
    articulo(Num, Gen, Det),
    nombre(Num, Gen, N).

% Permite nombres sin artículo (ej. "come comida")
sintagma_nominal_base(Num, Gen, sin_art, N) -->
    nombre(Num, Gen, N).

% Recursividad en Complementos
% Esto permite encadenar infinitos complementos al sustantivo usando formato de lista [Cabeza|Cola].
...
% Caso base de la recursión: la lista vacía detiene el bucle.
...
% La subordinada hereda el 'Num' del sustantivo que la precede para
% que su propio verbo también concuerde (ej: "el niño que corre", no "que corren").
...
% Estructura modular del SV: puede tener negación opcional, un núcleo y complementos.
...
%% Lista recursiva de complementos del verbo (Obj. Directo, Atributo, etc.)
...
% Predicados para dibujar el árbol.
mostrar_arbol(Termino) :-
    nl,
    write('Arbol visual:'), nl,
    imprimir_arbol(Termino, 0).

% Caso base: Si es un elemento atómico (una hoja, como 'juan' o 'corre'), lo imprime.
imprimir_arbol(Termino, Nivel) :-
    atomic(Termino),
    tab(Nivel),
    write('- '), write(Termino), nl.

% Si el término es compuesto (ej. sn(art(el), n(niño))), =.. lo rompe
% dinámicamente en una lista: [sn, art(el), n(niño)].
% Así imprimimos el functor (sn) y aplicamos recursión a los hijos (argumentos).
imprimir_arbol(Termino, Nivel) :-
    compound(Termino),
...
% Procesa la lista de hijos recursivamente.
imprimir_hijos([], _).
